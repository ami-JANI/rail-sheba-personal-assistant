...
In Dry run mode it only reports which seats it would choose. With Dry run disabled, it clicks seats one at a time and waits for Rail Sheba to mark each tile as selected before clicking the next. Any seats already confirmed on the page count toward the requested total on a rerun. You must then review the total and click **CONTINUE PURCHASE** manually.
